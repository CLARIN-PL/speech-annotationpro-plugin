...
Lexicon::Lexicon(const std::string& g2p_model_file, const std::string& phone_list_file) :
	...
	std::ifstream file(phone_list_file.c_str());
	std::string ph;
	while (!file.fail()) {
		getline(file, ph);
		if (!ph.empty())
	...
	for (const auto& word : wordlist) {
		...
		for (const auto& t : transcriptions) {
			...
			lexicon_fst.AddArc(cur_state,
				fst::StdArc(psyms.Find(pron.back()), output_word, no_sil_cost + pron_cost, loop_state));
